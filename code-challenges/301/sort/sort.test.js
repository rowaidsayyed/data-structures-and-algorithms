...
const sortMeetingsByDay = (arr) => {
  ...
  arr.sort((a, b) => {
    return days.indexOf(a.dayOfWeek.toLowerCase()) > days.indexOf(b.dayOfWeek.toLowerCase())?1:-1;
  });
  return arr;
};
...
const sortSchedule = (arr) => {
  ...
  let days = ['monday','tuesday','wednesday','thursday','friday'];
  arr.sort((a,b) => {
    if(days.indexOf(a.dayOfWeek.toLowerCase()) === days.indexOf(b.dayOfWeek.toLowerCase())){
      if(parseInt(a.start) > parseInt(b.start)) return true;
