...
Write a function named removeVowels that takes in a string and returns a new string where all the vowels of
the original string have been removed.
...
const removeVowels = (str) => {
  ...
  for(let i =0;i<str.length;i++){
    if(!vowelsArr.includes(str[i])){
      notVowArr.push(str.slice(i,i+1));
    }
  }
  return notVowArr.join('');
};

/* ------------------------------------------------------------------------------------------------
CHALLENGE 10 - Stretch Goal
Write a function named extractVowels that takes in a string and returns an array where the first element is
the original string with all the vowels removed,
...
const extractVowels = (str) => {
  ...
  for(let i =0;i<str.length;i++){
    ...
    if(vowelsArr.includes(str[i])){
      varr.push(str.slice(i,i+1));
    }
  }
  let allArr = [notVowArr.join(''),varr.sort().join('')];
  return allArr;
};
